...
from pygame.constants import K_w, K_a, K_s, K_d, K_i, K_j, K_l, K_k
...
class MultiAgentPuckWorld(PyGameWrapper):
    """
    ...
    def __init__(self,
        ...
        self.diag = math.sqrt(width * height)
        self.CREEP_BAD = {
        ...
        self.AGENT_INIT_POS = (
            ...
            self.AGENT_RADIUS * 1.5)
        if self.multiplayer:
            self.AGENT2_INIT_POS = (
                self.AGENT_RADIUS * 1.5 + self.width/2,
                self.AGENT_RADIUS * 1.5)
            self.AGENT2_COLOR = (60, 60, 60) # TODO change color
    ...
    def _handle_player_events(self):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if key == self.actions["left2"]:
                    self.dx2 -= self.AGENT_SPEED

                if key == self.actions["right2"]:
                    self.dx2 += self.AGENT_SPEED

                if key == self.actions["up2"]:
                    self.dy2 -= self.AGENT_SPEED
    ...
    def getGameState(self):
        ...
        if self.multiplayer:
            ma_state = {
                "player2_x": self.player2.pos.x,
                "player2_y": self.player2.pos.y,
                "player2_velocity_x": self.player2.vel.x,
                "player2_velocity_y": self.player2.vel.y
            }
            state.update(ma_state)
    ...
    def game_over(self):
        ...
        """
        if self.dist_to_bad < (self.CREEP_BAD['radius_outer']/2):
            return True
        if self.ticks == 2500:
            self.ticks = 0
            return True
    ...
    def step(self, dt):
        ...
        self.player.update(self.dx, self.dy, dt, simple=self.simple)
        ...
        self.player2.update(self.dx2, self.dy2, dt, simple=self.simple)
        dx2 = self.player2.pos.x - self.good_creep.pos.x
        dy2 = self.player2.pos.y - self.good_creep.pos.y
        dist_to_good2 = math.sqrt(dx2 * dx2 + dy2 * dy2)
        dx2 = self.player2.pos.x - self.bad_creep.pos.x
        dy2 = self.player2.pos.y - self.bad_creep.pos.y
        dist_to_bad2 = math.sqrt(dx2 * dx2 + dy2 * dy2)

        dist_to_good_min = min(dist_to_good, dist_to_good2)
        dist_to_bad_min = min(dist_to_bad, dist_to_bad2)

        self.dist_to_bad = dist_to_bad_min

        if dist_to_good_min < self.CREEP_BAD['radius_outer']:
            reward = self.diag - dist_to_good
        else:
            reward = (self.diag - dist_to_good) / 2.0
        if self.dist_to_bad < self.CREEP_BAD['radius_outer']:
            reward -= 10*dist_to_good_min
        ...
        dx_arr = [dx, dx2]
        dy_arr = [dy, dy2]
        dist_to_bad_arr = [dist_to_bad, dist_to_bad2]
        idx = np.argmin(dist_to_bad_arr)
        dx = dx_arr[idx]
        dy = dy_arr[idx]
        dist_to_bad = dist_to_bad_arr[idx]
    ...
    def new_step(self, dt):
        ...
        dy = self.player.pos.y - self.bad_creep.pos.y
        dist_to_bad = math.sqrt(dx * dx + dy * dy)
        self.dist_to_bad = dist_to_bad


        if dist_to_good < self.CREEP_BAD['radius_outer']:
            reward = self.diag - dist_to_good
        else:
            reward = (self.diag - dist_to_good) / 2.0
    ...
    def _setAction(self, actions, last_action):
        """
        ...
        if isinstance(actions, list):
            ...
            super(MultiAgentPuckWorld, self)._setAction(actions, last_action)
...
if __name__ == "__main__":
    ...
    pygame.init()
    game = PuckWorld(width=256, height=256, multiplayer=True)
