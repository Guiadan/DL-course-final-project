...
class PLE(object):
    ...
    def _oneStepAct(self, action):
        ...
        #print "DEBUG: ple - _oneStepAction - action - " + str(action)
        if isinstance(action, list):
            for i in range(len(action)):
                if action[i] not in self.getActionSet():
                    action[i] = self.NOOP
        else:
    ...
    def _setAction(self, action):
        ...
        """
        #print "DEBUG: ple - _setAction - action - " + str(action)
...
class MAPLE(PLE):
    def __init__(self,
                 ...
                 reward_values={}, force_fps=True, display_screen=False,
                 add_noop_action=True, state_preprocessor=None, rng=24):
        super(MAPLE, self).__init__(game, fps, frame_skip, num_steps,
                 reward_values, force_fps, display_screen,
                 add_noop_action, state_preprocessor, rng)
        
    
    def act(self, action):
        """
        Perform an action on the game by each agent. We lockstep frames with actions. If act is not called the game will not run.
