...
class Environment(object):
  def __init__(self, env_name, n_action_repeat, max_random_start,
               observation_dims, data_format, display, use_cumulated_reward=False, ple=False, ple_game_name='MultiAgentPuckWorld', ple_agents=1):
    if ple:
      self.env = PLEEnv(ple_game_name, display_screen=False, num_of_agents=ple_agents)
    else:
    ...
    self.ple = ple
    self.ple_game_name = ple_game_name
...
class AtariEnvironment(Environment):
  def __init__(self, env_name, n_action_repeat, max_random_start,
               observation_dims, data_format, display, use_cumulated_reward, ple=False, ple_game_name='MultiAgentPuckWorld', ple_agents=1):
    print "##########################" + str(ple_game_name) + "##########################"
    super(AtariEnvironment, self).__init__(env_name,
        n_action_repeat, max_random_start, observation_dims, data_format, display, use_cumulated_reward, ple, ple_game_name, ple_agents)
