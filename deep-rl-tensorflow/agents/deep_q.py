...
class DeepQ(Agent):
  def __init__(self, sess, pred_network, env, stat, conf, target_network=None, num_of_agents=1):
    super(DeepQ, self).__init__(sess, pred_network, env, stat, conf, target_network=target_network, num_of_agents=num_of_agents)
    self.t_next_target_network_update = self.t_target_q_update_freq + self.t_learn_start
    # Optimizer
    with tf.variable_scope('optimizer'):
      self.targets = tf.placeholder('float32', [None], name='target_q_t')
      self.actions = tf.placeholder('int64', [None], name='action')

      #print "DEBUG: self.pred_network.outputs - " + str(self.pred_network.outputs)
      actions_one_hot = tf.one_hot(self.actions, self.env.action_size/self.env.env.num_of_agents, 1.0, 0.0, name='action_one_hot')
  ...
  def observe(self, observation, reward, action, terminal):
    ...
    if self.t > self.t_learn_start:
      ...
      if self.target_q_update_freq_decay != 1:
        if self.t == self.t_next_target_network_update:
          self.update_target_q_network()
          self.t_target_q_update_freq = int(self.t_target_q_update_freq*self.target_q_update_freq_decay)
          self.t_next_target_network_update = self.t + self.t_target_q_update_freq
          print "target network updated at iteration: " + str(self.t)
      elif self.t % self.t_target_q_update_freq == self.t_target_q_update_freq - 1:
        self.update_target_q_network()
